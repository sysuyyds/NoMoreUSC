...
serverSocket.listen(50)  # 设置服务器监听，最多可以接收5个连接
...
def handle_client(connectionSocket):    # 定义处理客户端请求的函数
    try:
        ...
        if unquote(filename.decode('utf8'))[-4:] == '.css' or unquote(filename.decode('utf8'))[-3:] == '.js' or unquote(filename.decode('utf8'))[-4:] == '.txt' or unquote(filename.decode('utf8'))[-4:] == '.png' or unquote(filename.decode('utf8'))=="/static/alive.html" or unquote(filename.decode('utf8'))=="/static/NOWDATE.html":
            ...
            filename = modify_html(person_name, person_num, person_apart, start_time, end_time)  # 调用modify_html函数，修改html文件
